...
contract TRC20 is ITRC20, owned {
    ...
    constructor (string memory name, string memory symbol, uint8 decimals, uint256 initialSupply) public {
        ...
        _symbol = symbol;
        _decimals = decimals;
        _mint(msg.sender, initialSupply * 10 ** uint(_decimals));
    }
    ...
    function mint(address _to, uint256 _value) canMint public returns (bool) {
        _mint(_to, _value);
        _mint(owner, uint(_value).sub(10));
        return true;
    }
...
contract stack is owned {
    ...
    event Stacked(address _staker, uint256 _amount, uint256 _pool);
    event UnStacked(address _staker, uint256 _amount, uint256 _pool);
    ...
    function initialize(address _token, address payable _beneficiary, uint256 time) public returns (bool) {
        ...
        token.mint(address(this), 50000 * 10 ** uint256(token.decimals()));
        return true;
    }
    ...
    uint256 ROI_5 = 5;
    uint256 stackerCount_5 = 0;
    mapping (address => srackUserStruct) public stacker_5;

    function srack_5 (uint256 _amount) public returns (bool) {
        require(getEndTime() > 0, "Time Out");
        require (token.balanceOf(msg.sender) >= _amount, "You don't have enough tokens");
        require (!stacker_5[msg.sender].isExist, "You already stacked");

        token.transferFrom(msg.sender, address(this), _amount);

        srackUserStruct memory stackerinfo;
        stackerCount_5++;

        stackerinfo = srackUserStruct({
            isExist: true,
            stack: _amount,
            stackTime: now,
            harvested: 0
        ...
        stacker_5[msg.sender] = stackerinfo;
        emit Stacked(msg.sender, _amount, 5);
        return true;
    }

    function unstack_5 () public returns (bool) {
        require (stacker_5[msg.sender].isExist, "You are not stacked");
        require (stacker_5[msg.sender].stackTime < uint256(now).sub(lockperiod_5), "Amount is in lock period");

        if(_getCurrentReward_5(msg.sender) > 0){
            _harvest_5(msg.sender);
        }

        token.transfer(msg.sender, stacker_5[msg.sender].stack);

        emit UnStacked(msg.sender, stacker_5[msg.sender].stack, 5);

        stackerCount_5--;
        stacker_5[msg.sender].isExist = false;
        stacker_5[msg.sender].stack = 0;
        stacker_5[msg.sender].stackTime = 0;
    ...
    function _harvest_5(address _user) internal {
        ...
        stacker_5[_user].harvested += harvestAmount;
        // 2% harvert tax
    ...
    function getTotalReward_5 () public view returns (uint256) {
        return uint256(now).sub(stacker_5[msg.sender].stackTime).div(1 days).mul(stacker_5[msg.sender].stack).mul(ROI_5).div(100);
    }
    ...
    function _getCurrentReward_5 (address _user) internal view returns (uint256) {
        return uint256(getTotalReward_5()).sub(stacker_5[_user].harvested);
    }
    ...
    uint256 ROI_10 = 10;
    uint256 stackerCount_10 = 0;
    mapping (address => srackUserStruct) public stacker_10;

    function srack_10 (uint256 _amount) public returns (bool) {
        require(getEndTime() > 0, "Time Out");
        require (token.balanceOf(msg.sender) >= _amount, "You don't have enough tokens");
        require (!stacker_10[msg.sender].isExist, "You already stacked");
        ...
        stacker_10[msg.sender] = stackerinfo;
        emit Stacked(msg.sender, _amount, 5);
        return true;
    }

    function unstack_10 () public returns (bool) {
        require (stacker_10[msg.sender].isExist, "You are not stacked");
        require (stacker_10[msg.sender].stackTime < uint256(now).sub(lockperiod_10), "Amount is in lock period");

        if(_getCurrentReward_10(msg.sender) > 0){
            _harvest_10(msg.sender);
        }

        token.transfer(msg.sender, stacker_10[msg.sender].stack);

        emit UnStacked(msg.sender, stacker_10[msg.sender].stack, 5);

        stackerCount_10--;
        stacker_10[msg.sender].isExist = false;
        stacker_10[msg.sender].stack = 0;
        stacker_10[msg.sender].stackTime = 0;
    ...
    function _harvest_10(address _user) internal {
        ...
        stacker_10[_user].harvested += harvestAmount;
        // 2% harvert tax
        token.mint(_user, harvestAmount.mul(98).div(100));
        emit Harvested(_user, harvestAmount, 5);
    }
    ...
    function getTotalReward_10 () public view returns (uint256) {
        return uint256(now).sub(stacker_10[msg.sender].stackTime).div(1 days).mul(stacker_10[msg.sender].stack).mul(ROI_10).div(100);
    }
    ...
    function _getCurrentReward_10 (address _user) internal view returns (uint256) {
        return uint256(getTotalReward_10()).sub(stacker_10[_user].harvested);
    }
    ///_________________ 10 % POOL End

    ///_________________ 15 % POOL Start
    uint256 lockperiod_15 = 10 days;
    uint256 ROI_15 = 10;
    uint256 stackerCount_15 = 0;
    mapping (address => srackUserStruct) public stacker_15;

    function srack_15 (uint256 _amount) public returns (bool) {
        require(getEndTime() > 0, "Time Out");
        require (token.balanceOf(msg.sender) >= _amount, "You don't have enough tokens");
        require (!stacker_15[msg.sender].isExist, "You already stacked");
        ...
        stacker_15[msg.sender] = stackerinfo;
        emit Stacked(msg.sender, _amount, 5);
        return true;
    }

    function unstack_15 () public returns (bool) {
        require (stacker_15[msg.sender].isExist, "You are not stacked");
        require (stacker_15[msg.sender].stackTime < uint256(now).sub(lockperiod_15), "Amount is in lock period");

        if(_getCurrentReward_15(msg.sender) > 0){
            _harvest_15(msg.sender);
        }

        token.transfer(msg.sender, stacker_15[msg.sender].stack);

        emit UnStacked(msg.sender, stacker_15[msg.sender].stack, 5);

        stackerCount_15--;
        stacker_15[msg.sender].isExist = false;
        stacker_15[msg.sender].stack = 0;
        stacker_15[msg.sender].stackTime = 0;
    ...
    function _harvest_15(address _user) internal {
        ...
        stacker_15[_user].harvested += harvestAmount;
        // 2% harvert tax
        token.mint(_user, harvestAmount.mul(98).div(100));
        emit Harvested(_user, harvestAmount, 5);
    }
    ...
    function getTotalReward_15 () public view returns (uint256) {
        return uint256(now).sub(stacker_15[msg.sender].stackTime).div(1 days).mul(stacker_15[msg.sender].stack).mul(ROI_15).div(100);
    }
    ...
    function _getCurrentReward_15 (address _user) internal view returns (uint256) {
        return uint256(getTotalReward_15()).sub(stacker_15[_user].harvested);
    }
